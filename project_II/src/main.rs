...
fn main() {
    struct Company {
     ...
     let cadbury = Company {
        ...
        founding_date:1965,
        percentage_leverage:5_500_000/15_000_000
    };
     let champion = Company {
        ...
        founding_date:1974,
        percentage_leverage:8_000_000/25_000_000
    };
     let dangote = Company {
        ...
        founding_date:1970,
        percentage_leverage:10_000_000/18_000_000
    };
     let flour_mills = Company {
        ...
        founding_date:1960,
        percentage_leverage:4_000_000/32_000_000
    };
     let nestle = Company {
        ...
        founding_date:1961,
        percentage_leverage:1_500_000/8_000_000
    };
     let unilever = Company {
        ...
        founding_date:1923,
        percentage_leverage:11_000_000/37_000_000
    };
     let honeywell = Company {
        ...
        founding_date:1906,
        percentage_leverage:9_000_000/34_000_000
    };
     let nigerian_breweries = Company {
        ...
        founding_date:1946,
        percentage_leverage:12_000_000/30_000_000
    };
    loop 
{
        ...
        if username1.chars().count() <3 || username1.chars().count() > 8{
         ...
         else
         {
            ...
            'cadbry000' to access data for Cadbury
            'champn000' to access data for Champion
            'dango000' to access data for Dangote
            'flour000' to access data for Flour Mills
            'nest000' to access data for Nestle
            'unilvr000' to access data for Unilever
            'honey000' to access data for Honeywell
            'ngbrew000' to access data for Nigerian Breweries");
            io::stdin().read_line(&mut password).expect("Could not read input");
            let password1:&str = password.trim();
            if password1.chars().all(|a: char| a.is_ascii_lowercase() || a.is_digit(radix::10)) {
            ...
            let companies_name = vec!["Cadbury Nigeria Plc", "Champion Breweries Plc", "Dangote Sugar Refinery Plc", "Flour Mills Nigeria Plc", "Nestle Nigeria Plc", "Unilever Nigeria Plc", "Honeywell Nigeria Plc", "Nigerian Breweries Plc"];
            ...
            file.write_all(format!("{:<20} {:<20} {:<30} {:<30} {:<30}\n", "Company", "Founding Date", "Company Shares", "Company Liabilities", "Percentage Leverages").as_bytes()).expect("Error writing to file");
            for n in 0..companies_name.len() 
         {
            file.write_all(
            format!("{:<20} {:<20} {:<30} {:<30} {:<30}\n", companies_name[n], Company::founding_date[n], Company::shares[n], Company::liabilities[n], Company::percentage_leverage[n]).as_bytes()).expect("Error writing to file");
         }
